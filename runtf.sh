...
EOF
        ...
        2)
            ...
            full_storage_name="${storage_account}"
...
plan() {
    ...
    terraform -chdir="$TERRAFORM_DIR" show -json tf.plan | jq > tf_plan.json
...
checkov() {
    ...
    docker run --rm -t \
        ...
        --quiet \
        --compact \
        --soft-fail 2>/dev/null | grep -v "WARNING\|MainThread"
}
tfsec() {
    echo -e "${YELLOW}Executando análise de segurança...${NC}"
    docker run --rm -it -v "$(pwd)/$TERRAFORM_DIR:/src" aquasec/tfsec --force-all-dirs --soft-fail --concise-output /src 
}
tflint() {
...
setup_docs_structure() {
    ...
    mkdir -p docs/architecture/diagrams docs/guides docs/operations docs/security docs/images

    # Criando arquivo header
    cat > docs/.header.md << 'EOF'
# Módulo Terraform: Nome do Projeto

![Terraform](https://img.shields.io/badge/terraform-%235835CC.svg?style=for-the-badge&logo=terraform&logoColor=white)

Este módulo é responsável por criar recursos na Microsoft Azure para Ambiente < AMBIENT >.
...
- Permissões necessárias para o Service Principal:
  - Contributor - Utilizada pelo SP para provisionar recursos a nível de Subscription
  - Role Based Access Control Administrator - Utilizada pelo SP para atribuir permissões necessárias
- Conta de armazenamento do Azure configurada para armazenar o estado do Terraform
EOF

    # Criando arquivo footer
    cat > docs/.footer.md << 'EOF'
## Contribuição
...
EOF
    ...
    # Criando arquivo terraform-docs.yml
...
EOF
    ...
    echo -e "${GREEN}Estrutura de documentação configurada com sucesso!${NC}"
}
...
gera_diagram() {
    echo -e "${YELLOW}Gerando diagrama do projeto...${NC}"
    
    # Cria diretório se não existir
    mkdir -p docs/architecture/diagrams
    
    # Gera o arquivo .tfstate em formato JSON
    terraform show -json > tfstate.json
    
    # Usa o Terraform Graph para gerar o DOT
    terraform graph > graph.dot
    
    # Gera o diagrama usando Mermaid CLI
    docker run --rm -v "$(pwd):/data" \
        minlag/mermaid-cli:latest \
        -i docs/architecture/diagrams/mermaid.mmd \
        -o docs/architecture/diagrams/infrastructure_diagram.png
    
    echo -e "${GREEN}Diagramas gerados em:${NC}"
    echo -e "${GREEN}- docs/architecture/diagrams/infrastructure.png${NC}"
    echo -e "${GREEN}- docs/architecture/diagrams/infrastructure_diagram.png${NC}"
    
    # Limpa arquivos temporários
    rm -f tfstate.json graph.dot
}
...
main() {
    if [ $# -eq 0 ]; then
        ...
        echo -e "    gera_diagram         - Gera o diagrama do projeto"
        exit 1
    fi
